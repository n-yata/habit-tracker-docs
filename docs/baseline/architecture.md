...
| レイヤー | 役割 | 依存可能 | 依存禁止 |
|---|---|---|---|
| ページ（app） | ルーティング・描画 | 表示コンポーネント, データフック | データ取得の直接実装（フック経由にする） |
| 表示コンポーネント | 表示専用UI | UI プリミティブ, 生成型 | データフックのロジック実体 |
| データフック（features） | データ取得・機能ロジック | 基盤（lib）, 生成型 | ページへの逆依存 |
| 基盤（lib） | 横断基盤（API クライアント等） | 生成型 | features, app |
| 生成型（generated） | 契約由来の型（自動生成） | — | 手編集禁止 |
...
### CORS
- 許可オリジンは環境変数 `FRONTEND_ORIGIN`（backend）で指定し、コードにハードコードしない。ワイルドカード
  （`*` や部分ワイルドカード）は起動時に拒否する。
- 認証機構を持たない（上記）ため `AllowCredentials` は使用しない。将来認証を導入する際に見直す。

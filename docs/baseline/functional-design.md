...
（概念モデルの camelCase 名と物理カラム snake_case は対応する。例: `recurrenceType` ↔ `recurrence_type`）。
...
（テーブル定義書）を正本とする。以下は DB では担保せず、ドメイン層で扱う意味・ルール:
...
エンティティ・属性は日本語の論理名で示す（物理名との対応は
`docs/specs/3_detail-design/db/table-definition.md` を参照）。
...
第一マイルストーンの前提として、以下を確定する（他ドキュメントが前提とする、ぶれてはいけない決定）。
カラムの型・物理制約名・索引名・DDL生成方針などの物理的な詳細は
`docs/specs/3_detail-design/db/table-definition.md`（テーブル定義書）を正本とする。

- 日付・時刻は**サーバーローカル時刻基準**で扱う（第一マイルストーンは単一ユーザー・ローカル前提のため
  ユーザーTZは持たない）。
- 習慣の削除は物理削除ではなく論理削除（`archived=true`）で行い、過去のチェックイン記録との整合を保つ。
- チェックイン記録は**習慣と対象日の組で一意**に定まる（同一習慣・同一日付の重複を許さない）。
- リマインド超過判定は、**当日の対象習慣のみを対象に、サーバー時刻基準**で行う（過去日は対象外）。
...
**日付・時刻はサーバーローカル時刻基準**で扱う（データモデルの「確定事項（概念レベル）」参照）。

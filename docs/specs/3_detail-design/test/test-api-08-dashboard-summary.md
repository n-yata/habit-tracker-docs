...
| No | 観点 | 前提・入力 | 期待結果 | 完了 |
|---|---|---|---|:--:|
| 1 | 正常系 | 月水金習慣（`specific_days`）、火曜・木曜を挟んで月水金すべて `done` | 対象外日を挟んでも `current` が途切れず継続する | [ ] |
| 2 | 正常系 | 対象日（月）が `not_done` | その日で `current` が0にリセットされる | [ ] |
| 3 | 正常系 | `today` が対象日だが未記録 | 未達成による途切れとみなさない（中立扱い。直前までの `current` を維持） | [ ] |
| 4 | 正常系 | 過去の記録を `not_done`→`done` に修正 | 修正後、`current`/`longest` が再計算された値になる | [ ] |
| 5 | 正常系 | `weekly_count` 習慣（`weeklyTargetCount=3`）、ある週の `done` が3件以上 | その週が達成週としてカウントされる | [ ] |
| 6 | 正常系 | `weekly_count` 習慣、今週（進行中）が未達成 | 今週の未達成では `current` を途切れさせない（進行中のため） | [ ] |

## テストケース一覧: Domain `calcAchievementRate`

| No | 観点 | 前提・入力 | 期待結果 | 完了 |
|---|---|---|---|:--:|
| 7 | 正常系 | 月水金習慣、期間内対象日12日中9日 `done` | `0.75`（75%） | [ ] |
| 8 | 境界値 | 期間内に対象日が0件 | `null`（「対象なし」。`0` ではない） | [ ] |
| 9 | 正常系 | `weekly_count` 習慣、期間内対象週4週中3週達成 | `0.75` | [ ] |

## テストケース一覧: ヒートマップ state 分類

| No | 観点 | 前提・入力 | 期待結果 | 完了 |
|---|---|---|---|:--:|
| 10 | 正常系 | 対象日かつ `done` | `state="done"` | [ ] |
| 11 | 正常系 | 対象日かつ `not_done` | `state="missed"` | [ ] |
| 12 | 正常系 | 対象日かつ未記録 | `state="unrecorded"` | [ ] |
| 13 | 正常系 | 非対象日 | `state="not_target"` | [ ] |

## テストケース一覧: API-08（`GET /dashboard?from=&to=`）

| No | 観点 | 前提・入力 | 期待結果 | 完了 |
|---|---|---|---|:--:|
| 14 | 正常系 | 複数の習慣が登録済み | 習慣ごとに集約結果（`currentStreak`/`longestStreak`/`achievementRate`/`heatmap`）が返る | [ ] |
| 15 | 異常系 | `from > to` | `400`「from/to の指定が不正です」 | [ ] |
| 16 | 異常系 | `from`/`to` が不正形式 | `400` | [ ] |

...
| No | 観点 | 前提・入力 | 期待結果 | 完了 |
|---|---|---|---|:--:|
| 1 | 正常系 | 未記録の対象日に `status="done"` で記録 | `200`、新規 `CheckIn` が作成される | [ ] |
| 2 | 正常系 | 既存記録（`done`）を `status="not_done"` で修正 | `200`、既存レコードの `status` と `updatedAt` のみ更新（新規行が増えない） | [ ] |
| 3 | 異常系 | 存在しない `id` | `404`「対象の習慣が見つかりません」 | [ ] |
| 4 | 異常系 | 月水金習慣（`specific_days`, `targetWeekdays=[1,3,5]`）の火曜日に記録 | `422`「その日はこの習慣の対象ではありません」 | [ ] |
| 5 | 異常系 | `status="maybe"`（不正な値） | `400`「status は done または not_done を指定してください」 | [ ] |
| 6 | 異常系 | `date` が `YYYY-MM-DD` 形式でない | `400` | [ ] |

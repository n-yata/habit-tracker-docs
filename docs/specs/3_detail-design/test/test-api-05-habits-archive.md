...
| No | 観点 | 前提・入力 | 期待結果 | 完了 |
|---|---|---|---|:--:|
| 1 | 正常系 | 存在する `id` をアーカイブ | `204`。アーカイブ後、API-01（一覧）の結果に含まれない | [ ] |
| 2 | 正常系 | アーカイブ後 | API-03（詳細取得）で `archived=true` として取得可能（物理削除されていない） | [ ] |
| 3 | 正常系 | アーカイブ対象に紐づく `check_ins` レコードが存在する状態でアーカイブ | アーカイブ後も `check_ins` レコードが削除されずに残る（論理削除の確認） | [ ] |
| 4 | 異常系 | 存在しない `id` | `404`「対象の習慣が見つかりません」 | [ ] |
| 5 | 境界値 | 既に `archived=true` の `id` に再度 `DELETE` | `204`（冪等に成功する） | [ ] |

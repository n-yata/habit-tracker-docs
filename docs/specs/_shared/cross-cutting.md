...
> 機能設計書の一部。親: [`functional-design.md`](../../baseline/functional-design.md)。
> 個別レイヤーに閉じない横断的な設計をまとめる。フェーズ固有のディレクトリ（`2_basic-design/` 等）に
> 属さない、横断的に参照する実装詳細として `docs/specs/_shared/` に置く。
...
各日の `state`（baseline の「アルゴリズム設計（ドメインロジック）」のヒートマップ用データ）に対応:
...
| 非対象日への記録 | 422 | 「その日はこの習慣の対象ではありません」 |
| DB障害 | 500、リトライ不可はログ記録 | 「一時的なエラーが発生しました」 |

HTTP ステータスの割り当ては [`api-design.md`](../2_basic-design/api-design.md) と対応する。

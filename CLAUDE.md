...
2. **実装詳細（`docs/specs/` に退避してよい）** — JSON入出力例、レイヤー別の擬似インターフェース、
   ...
   - 例（`functional-design.md`）: API一覧表・画面一覧+遷移図・ユースケース一覧・データモデル
     （**ER図・区分値等の概念モデルまで**）・中核アルゴリズムは baseline に残す。各APIのJSON例・
...
3. **基本設計と詳細設計の境界** — **論理・外部から見た設計＝基本設計**（データモデルのER図、
   ...
   - 例: `functional-design.md`（基本設計の正本）と `repository-structure.md`（具体的な
     ディレクトリ構造＝詳細設計）。
   - 例: データモデルの **ER図・概念モデル（エンティティ・区分値）は基本設計**（baseline）、
     **物理スキーマ（カラム定義・型・物理制約名・索引名）は詳細設計**
     （`docs/specs/3_detail-design/db/table-definition.md`）。物理DDL/マイグレーションSQLは
     さらにその先の実装。
